...
void UART_InitWithInt(unsigned int uiBaudRate){
   ...
   // UART0
   PINSEL0 = PINSEL0 | UART0RxD;                                // ustawic pina na odbiornik uart0
   U0LCR  |= m8BIT_UART_WORD_LENGTH | mDIVISOR_LATCH_ACCES_BIT; // dlugosc slowa, DLAB = 1
   U0DLL   = (((15000000)/16)/uiBaudRate);                    // predkosc transmisji
   U0LCR  &= (~mDIVISOR_LATCH_ACCES_BIT);                       // DLAB = 0
   U0IER  |= mRX_DATA_AVALIABLE_INTERRUPT_ENABLE;               // ??? co tu robimy - inicjalizuje przerwania na linii Rx
...
void Receiver_PutCharacterToBuffer(char cCharacter) {
	...
	if(sRxBuffer.ucCharCtr > (RECEIVER_SIZE-1)){
