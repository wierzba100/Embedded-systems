...
void LedStep(enum eSide Dirrection){
	if(Dirrection == LEFT){
		LedOn(uiCounter % 4);
		uiCounter++;}
	else if(RIGHT){
...
int main(){
	unsigned char ucStepCounter1;
	unsigned char ucStepCounter2;
	
	while(1){
		for(ucStepCounter1=1; ucStepCounter1<9;ucStepCounter1++){
		LedStep(RIGHT);
		Delay(50);}
		for(ucStepCounter2=1; ucStepCounter2<9;ucStepCounter2++){
		LedStep(LEFT);
		Delay(50);}
	}
}
